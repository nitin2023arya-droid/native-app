...
>
 ...
 renderList(entries, customer) {
    ...
    // ---- compute running full balances for this customer ----
    ...
    return entries.map(e => {
        const fullBalance = balanceMap[e.id] || { gold: { Premium:0, Mcx:0, Weight:0 }, silver: { Premium:0, Mcx:0, Weight:0 }, cash:0 };
        const cashAfter = fullBalance.cash || 0;
        const cashDisplay = ui.fmtBalHTML(cashAfter, 0, '');

        const goldTotal = (fullBalance.gold.Premium || 0) + (fullBalance.gold.Mcx || 0) + (fullBalance.gold.Weight || 0);
        const silverTotal = (fullBalance.silver.Premium || 0) + (fullBalance.silver.Mcx || 0) + (fullBalance.silver.Weight || 0);

        if (e.nature === 'Sauda') {
            const pendingWeight = ui.getSaudaPendingWeight(customer, e.id);
            return `
            <div class="card">
  ...
  <path d="M9 6V4h6v2"/>
            ...
            // ---- non-Sauda entries (keep original structure, just add stock balances) ----
            let entryHtml = '';
                    ...
                    <div style="color:var(--muted); font-size:0.85rem; margin-top:8px; line-height:1.5;">
                        ...
                        <div class="flex j-bet" style="margin-top:2px;">
                            ...
                            <span>Fine: ${e.fine || 0}g</span>
                                    ...
                                    <div style="margin-top:12px; padding:6px; background:#121826; border-radius:4px; border-color:#ffffff; font-size:0.85rem;">
                        <div style="display:flex; justify-content:space-between;">
                            <span>Cash  &nbsp;after entry:</span>
                            <span>${cashDisplay}</span>
                        </div>
     
                        <div style="display:flex; justify-content:space-between;">
                            <span><span style="color:var(--gold)">Gold</span>&nbsp;&nbsp; after entry:</span>
                            <span>${ui.fmtBalHTML(goldTotal, 3, 'g')}</span>
                        </div>
                        <div style="display:flex; justify-content:space-between; margin-top:2px;">
                            <span><span style="color:var(--silver)">Silver</span>&nbsp;&nbsp;after entry: </span>
                            <span>${ui.fmtBalHTML(silverTotal, 3, 'g')}</span>
                        </div>
                    </div>

                    <div class="flex gap-1" style="margin-top:10px">
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showEntryModal('Physical','${e.id}')">Edit</button>
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showDateEditModal('${e.id}')">Date</button>
                        <button class="btn btn-dr" style="flex:1; font-size:0.7rem" onclick="ui.deleteEntry('${e.id}')">Delete</button>
                    </div>
                </div>`;
            }
            ...
            ========================= */
                ...
                const prevCash = prevEntry ? (balanceMap[prevEntry.id]?.cash || 0) : 0;
                const isDr = cashAfter < prevCash;
                ...
                <div class="card">
                    ...
                    <div class="flex gap-1" style="margin-top:10px">
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showEntryModal('Round-off','${e.id}')">Edit</button>
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showDateEditModal('${e.id}')">Date</button>
                        <button class="btn btn-dr" style="flex:1; font-size:0.7rem" onclick="ui.deleteEntry('${e.id}')">Delete</button>
                    </div>
                </div>`;
            ...
            ========================= */
                ...
                <div class="card">
                    ...
                    <div style="color:var(--muted); font-size:0.85rem; margin-top:8px; line-height:1.5;">
                        ...
                        <div class="flex j-bet" style="margin-top:2px;">
                            <span>${e.purity !== undefined ? `Purity: ${e.purity}%` : ''}</span>
                            <span>Fine: ${e.fine || 0}g</span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-1" style="margin-top:10px">
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showEntryModal('Paid','${e.id}')">Edit</button>
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showDateEditModal('${e.id}')">Date</button>
                        <button class="btn btn-dr" style="flex:1; font-size:0.7rem" onclick="ui.deleteEntry('${e.id}')">Delete</button>
                    </div>
                </div>`;
            ...
            ========================= */
                ...
                const settlementAction = bookingAction === 'Purchase (IN)' ? 'Sale (OUT)' : bookingAction === 'Sale (OUT)' ? 'Purchase (IN)' : '';
                ...
                <div class="card">
                    ...
                    <div class="flex gap-1" style="margin-top:10px">
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showEntryModal('Settle','${e.id}')">Edit</button>
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showDateEditModal('${e.id}')">Date</button>
                        <button class="btn btn-dr" style="flex:1; font-size:0.7rem" onclick="ui.deleteEntry('${e.id}')">Delete</button>
                    </div>
                </div>`;
            ...
            ========================= */
                ...
                <div class="card">
                    ...
                    <div class="flex gap-1" style="margin-top:10px">
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showEntryModal('Cash','${e.id}')">Edit</button>
                        <button class="btn btn-s" style="flex:1; font-size:0.7rem" onclick="ui.showDateEditModal('${e.id}')">Date</button>
                        <button class="btn btn-dr" style="flex:1; font-size:0.7rem" onclick="ui.deleteEntry('${e.id}')">Delete</button>
                    </div>
                </div>`;
